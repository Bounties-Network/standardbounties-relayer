...
if (process.env.MNEMONIC) {
  ...
  web3 = new Web3(
    new HDWalletProvider(process.env.MNEMONIC, process.env.INFURA_HTTP_PROVIDER)
  );
} else {
  web3 = new Web3(
    new Web3.providers.HttpProvider(process.env.INFURA_HTTP_PROVIDER)
  );
}
...
async function getAccountBalance(account) {
  ...
  console.log(
    "Relay account current balance:",
    `${balance} Wei = ${web3.utils.fromWei(balance, "ether")} ETH`
  );
  return balance;
}
...
web3.eth.getAccounts().then(async _accounts => {
  ...
  if (!hasEnoughBalance(accounts[process.env.RELAYER_ACC_INDEX], "0")) {
...
const contractsPromise = new Promise(async (resolve, reject) => {
  try {
    ...
    let StandardBounties = await new web3.eth.Contract(
      standardBounties.abi,
      process.env.STANDARD_BOUNTIES_ADDRESS
    );
...
app.get("/relay/list/:address", async (req, res) => {
  ...
  let result = {
    ...
    nonce: web3.utils.hexToNumber(
      await BountiesMetaTxRelayer.methods.replayNonce(req.params.address).call()
    )
  };
...
app.get("/relay/nonce/:account", async (req, res) => {
  ...
  if (req.params.account) {
    latestNonce = await BountiesMetaTxRelayer.methods
      .replayNonce(req.params.account)
      .call();
  }
  res.end(
    JSON.stringify({
 ...
 *
 */
app.post("/relaydemo", async (req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.set("Content-Type", "application/json");
  ...
  if (!cache) {
    ...
    const latestNonce = await BountiesMetaTxRelayer.methods
      .replayNonce(req.body.sender)
      .call();
    ...
    if (signer == sender) {
      ...
      BountiesMetaTxRelayer.methods
        ...
        .on("receipt", receipt =>
          console.log("metaFulfillBounty Receipt", receipt)
        );
    } else {
      res.end(
        JSON.stringify({
    ...
    res.end(
      JSON.stringify({
        status: 400,
        message:
          "You have reached the limited quota for RelayedTx. Try again later."
      })
    );
  }
});
  ...
  if (!cache) {
    ...
    if (signer == sender) {
      ...
      try {
        const estimateGas = await BountiesMetaTxRelayer.methods[
          req.body.method
        ](signature, ...methodParams).estimateGas({ from: RELAYER_ADDRESS });
        ...
        BountiesMetaTxRelayer.methods[method](signature, ...methodParams)
          ...
          .on("error", (error, receipt) => {
            console.log(
              `ERROR while executing method ${method}`,
              error,
              receipt
            );
            res.end(
