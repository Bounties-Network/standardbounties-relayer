  ...
  try {
    ...
    let result = {
      ...
      nonce: web3.utils.hexToNumber(
        await BountiesMetaTxRelayer.methods
          .replayNonce(req.params.address)
          .call()
      )
    };
  ...
  try {
    ...
    if (!cache) {
      ...
      if (signer == sender) {
        ...
        const methodParams = _.map(
          _.filter(params, (obj, i) => i > 1),
          o => o.v
        );
...
(async () => {
  ...
  if (process.env.MNEMONIC) {
    ...
    web3 = new Web3(
      new HDWalletProvider(
        ...
        process.env.INFURA_HTTP_PROVIDER
      )
    );
  } else {
    web3 = new Web3(
  ...
  web3.eth
    ...
    .then(async _accounts => {
      ...
      const balanceResultGT = await hasEnoughBalance(
        accounts[process.env.RELAYER_ACC_INDEX],
        "0"
      );
